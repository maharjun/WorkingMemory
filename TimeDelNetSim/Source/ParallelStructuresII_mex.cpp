...
void CurrentUpdate::operator () (const tbb::blocked_range<int*> &BlockedRange) const{
	...
	for (int * iter = begin; iter < end; ++iter){
		...
		long long int AddedCurrentLL = (long long int)(CurrentSynapse.Weight*((1 + CurrRelativeInc > 0)? 1 + CurrRelativeInc:0) * (1LL << 32));
...
void NeuronSimulate::operator() (tbb::blocked_range<int> &Range) const{
	...
	for (size_t j = RangeBeg; j < RangeEnd; ++j){
		if (Vnow[j] == 30.0f){
			...
			float IinCurr = (float)(Iin[j]) / (1LL << 32);
			Iin[j] = (long long)(((IinCurr > IinMax) ? IinMax : IinCurr)*(1LL << 32));
