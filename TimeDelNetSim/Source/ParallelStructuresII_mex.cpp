...
void CurrentUpdate::operator () (const tbb::blocked_range<int32_t*> &BlockedRange) const{
	...
	for (int32_t * iter = begin; iter < end; ++iter){
		...
		int64_t AddedCurrentLL = (int64_t)(CurrentSynapse.Weight * (1LL<<32));
		Iin1[CurrentSynapse.NEnd - 1].fetch_and_add(AddedCurrentLL);
		Iin2[CurrentSynapse.NEnd - 1].fetch_and_add(AddedCurrentLL);
...
void NeuronSimulate::operator() (tbb::blocked_range<int> &Range) const{
	...
	for (size_t j = RangeBeg; j < RangeEnd; ++j){
		if (Vnow[j] == 30.0f){
			...
			float Vnew, Unew;
			Vnew = Vnow[j] + (Vnow[j] * (0.04f*Vnow[j] + 5.0f) + 140.0f - Unow[j] + (float)(Iin2[j] - Iin1[j]) / (1LL << 32) + Iext[j]) / onemsbyTstep;
...
void CurrentAttenuate::operator() (tbb::blocked_range<int> &Range) const {
	...
	tbb::atomic<int64_t> *Begin1 = &Iin1[Range.begin()];
	tbb::atomic<int64_t> *End1 = &Iin1[Range.end()-1] + 1;
	tbb::atomic<int64_t> *Begin2 = &Iin2[Range.begin()];
	tbb::atomic<int64_t> *End2 = &Iin2[Range.end() - 1] + 1;

	for (tbb::atomic<int64_t> *i = Begin1, *j = Begin2; i < End1; ++i, ++j){
		(*i) = (int64_t)(float(i->load()) * attenFactor1);
		(*j) = (int64_t)(float(j->load()) * attenFactor2);
	}
}
...
void InternalVars::DoOutput(StateVarsOutStruct &StateOut, OutputVarsStruct &OutVars) {
	...
	if (StorageStepSize && (Time % (StorageStepSize*onemsbyTstep) == 0) || !StorageStepSize) {
		...
		if (OutputControl & OutOps::I_IN_1_REQ) {
			...
			for (int j = 0; j < N; ++j)
				StateOut.Iin1Out.lastRow()[j] = (float)Iin1[j] / (1LL << 32);
		}
		if (OutputControl & OutOps::I_IN_2_REQ) {
			StateOut.Iin2Out.push_row_size(1);
			for (int j = 0; j < N; ++j)
				StateOut.Iin2Out.lastRow()[j] = (float)Iin2[j] / (1LL << 32);
		}
		...
		if (OutputControl & OutOps::I_IN_REQ) {
			...
			for (int j = 0; j < N; ++j)
				OutVars.Iin.lastRow()[j] = (float)(Iin2[j] - Iin1[j]) / (1LL << 32);
		}
		...
		if (OutputControl & OutOps::I_TOT_REQ) {
			...
			for (int j = 0; j < N; ++j)
				OutVars.Itot.lastRow()[j] = this->IextInterface.Iext[j] + (float)(Iin2[j] - Iin1[j]) / (1LL << 32);
		}
...
void InternalVars::DoSingleStateOutput(SingleStateStruct &SingleStateOut){
	...
	for (int j = 0; j < N; ++j){
		SingleStateOut.Iin1[j] = (float)Iin1[j] / (1LL << 32);
		SingleStateOut.Iin2[j] = (float)Iin2[j] / (1LL << 32);
	}
